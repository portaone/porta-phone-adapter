...
`patch_voicemail_message` and `delete_voicemail_message` have no such defence and no
response to check an id against: if PortaBilling resolves a stale uid the same way
there, the adapter flags or permanently deletes a different message and reports
success. Closing that needs a pre-verification call, which WT-1992 did not cover.
